...
namespace SmsParser
{
    ...
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(ExcelWriter).Name);
        ...
        public void ExportSmsInfo(List<SmsInfo> listSmsInfo, string filePath)
        {
            ...
            sheet.Columns[colHash["body"] + 1].ColumnWidth = Properties.Settings.Default.BodyColumnWidth;
            ...
            //process bank info
            Worksheet sheet2 = workbook.Worksheets.get_Item(2);
            sheet2.Name = DateTime.Now.ToString("Bank");

            numRows = listSmsInfo.Count(x => x.Bank != null && x.Bank.ParseStatus == StatusBankInfo.Okay) + 1;
            ...
            {
                if (item.Bank != null && item.Bank.ParseStatus == StatusBankInfo.Okay)
                {
            ...
            sheet2.Columns[6].ColumnWidth = Properties.Settings.Default.BodyColumnWidth;
